...
export const fullFunctionName = (
  packageId: string,
  moduleOrFunctionName: string,
  functionName?: string
): `${string}::${string}::${string}` => {
  if (functionName) {
    // If 3 arguments are provided (packageId, moduleName, functionName)
    return `${packageId}::${moduleOrFunctionName}::${functionName}`
  } else {
    // If 2 arguments are provided (packageId, functionName)
    return `${fullModuleName(packageId)}::${moduleOrFunctionName}`
  }
}
